			...
			<div class="box">
				<div class="cont">
					<div class="step-one" v-show="isTrue">
						...
						<div v-show="activityId" class="btn" @click="openBag">点击开奖</div :alertShow='alertShow'>
					</div>
					<div class="step-two" v-show="!isTrue">
						...
						<a v-show=false href="#/history">查看我的钱包</a>
					</div>
					<div v-show=false class="close" @click="order"></div>				
				</div>
			</div>
		</div>
	...
	export default{
		...
		methods:{
			...
			getActivityId:function(){
				var self = this;
				$.get('http://mall.mlxing.com/api/prize/getActivity',function(res){
					//console.log(res);
					if(res.code==0){
						self.activityId = res.response.id
						$('h2').html('抽奖');
						if(localStorage.aid){
							if(localStorage.aid != res.response.id){
								localStorage.prize = '';
								localStorage.aid = '';
							}
						}
			...
			openBag:function(){
				...
				if(localStorage.prize){
					this.isTrue = false;
					this.price = localStorage.prize;
					return false;
				}
				var self = this;
				this.isTrue = false;
				//this.price = (Math.random()*10).toFixed(2);
				//this.mid = this.$store.state.mid;
				//this.mid = Math.ceil(Math.random()*100+1);
				var obj = {
					"activityId":this.activityId,
					"userNo":this.mid,
				}
				$.ajax({
					url:'http://mall.mlxing.com/api/prize/goLottery',
					data:obj,
					dataType:'jsonp',
					success:function(res){
						if(res.code==0){
							self.price = res.response.prizeName;
							if(window.localStorage){
								localStorage.prize = res.response.prizeName;
								localStorage.aid = res.response.activityId;
							}else{
