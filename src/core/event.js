...
define(function(require, exports, module) {
    ...
    var _ = require("./lang"), slice = Array.prototype.slice, ARGS_RE = /\S+/g;
    ...
    var Handlers = function() {
        ...
        function Constructor(options) {
            ...
            this.options = typeof options === "string" ? optionsCache[options] || createOptions(options) : _.extend({}, options);
            // handlers' list
            this.list = [];
        ...
        Constructor.prototype = {
            _fire: function(data) {
                var list = this.list, stack = this.stack;
            ...
            has: function(handler) {
                var list = this.list;
                return handler ? _.indexOf(list, handler) > -1 : !!(list && list.length);
            },
            empty: function() {
                this.list.length = 0;
                this.firingLength = 0;
            ...
            fireWith: function(context, args) {
                var notFired = this.status !== STATUS.FIRED, firing = this.status === STATUS.FIRING, list = this.list, stack = this.stack;
                args = args || [];
    ...
    var Promise = function() {
        var STATUS = {
            ...
            RESOLVED: 1,
            REJECTED: 2
        }, configs = [ [ "resolve", "done", STATUS.RESOLVED ], [ "reject", "fail", STATUS.REJECTED ], [ "notify", "progress" ] ];
        function Constructor(obj) {
            this.handlers = [ // doneHandlers
            new Handlers("once memory"), // failHandlers
            new Handlers("once memory"), // progressHandlers
            new Handlers("memory") ];
            this.status = STATUS.INIT;
            // promise['done' | 'fail' | 'progress']
            _.each(configs, function(cfg, i) {
                var that = this, handlers = this.handlers[i], status = cfg[2];
                this[cfg[1]] = function() {
        ...
        function when() {
            var completed = [], multiArgs = [], multiPromise = new Promise(), resolve = multiPromise.resolve;
            multiPromise._when = [];
            ...
            _.each([ "done", "fail", "progress", "always", "resolve", "reject", "notify" ], function(method) {
    ...
    Event.prototype.on = function(events) {
        var args = slice.call(arguments, 1), iterator;
