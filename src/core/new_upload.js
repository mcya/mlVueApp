...
function send_request()
{
    ...
    if (xmlhttp!=null)
    {
        serverUrl = 'http://weixin.mlxing.com/oss/sign';
...
var uploader1 = new plupload.Uploader({
	...
	init: {
		...
		FilesAdded: function(up, files) {
            ...
            if(window.location.hash.split('/')[1] == "find_guide"){
                var vm = window.find_guide;
                vm.changeCanvas(files);//调用显示的函数
            }else{
                //console.log(files);
                var vm = window.profile;
                vm.uploadImg1(files);
            }
		...
		BeforeUpload: function(up, file) {
            ...
            window.bgFileName = get_uploaded_object_name(file.name);
        },
		...
		FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                ...
                if(window.location.hash.split('/')[1] == "find_guide"){
                    //console.log('aaa');
                    var vm = window.find_guide;
                    vm.imgUploadsuccess(get_uploaded_object_name(file.name));//开始提交代码
                }else{
                    //console.log('传图成功'+get_uploaded_object_name(file.name));
                    var vm = window.profile;
                    vm.imgUploadsuccess1(get_uploaded_object_name(file.name));//开始提交代码
                }
...
var uploader2 = new plupload.Uploader({
    ...
    init: {
        ...
        BeforeUpload: function(up, file) {
            ...
            check_object_radio();
            set_upload_param(up, file.name, true);
            window.headFileName = get_uploaded_object_name(file.name);
        },
        ...
        FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                ...
                if(window.location.hash.split('/')[1] == "find_guide"){
                    ...
                    var vm = window.profile;
                    vm.imgUploadsuccess2(get_uploaded_object_name(file.name));//开始提交代码
                }
                // document.getElementById(file.id).getElementsByTagName('b')[0].innerHTML = 'upload to oss success, object name:' + get_uploaded_object_name(file.name);
            }
            else
