...
define(function (require, exports) {
    ...
    _.each(["get", "post", "put", "delete"], function (m) {
        exports[m] = function () {
            return ajax(_.extend(createOptions(arguments), {
                type: m
            }));
        };
    });
    ...
    exports.sendForm = function () {
        var fd, options, f = arguments[0], params = slice.call(arguments, 1);
    ...
    exports.sendFile = function () {
        ...
        return ajax(_.extend(options, {
            type: "post",
            headers: {
                "X-File-Name": options.data.name
            }
        }));
    };
    jsonp = exports.jsonp = function () {
        var options = createOptions(arguments), xhr = {
            id: ++id
        }, callbackName = "way_jsonp_" + xhr.id, script = document.createElement("script"), settings = xhr.ajaxSettings = _.extend({}, defaults, options), url = settings.url, promise = xhr.promise = new Promise(), oldAlways = promise.always;
    ...
    ajax = exports.ajax = function (options) {
        ...
        var xhr = new XMLHttpRequest(), upload = xhr.upload, tmp, settings = _.extend({}, defaults, options), type = settings.type = settings.type.toLowerCase(), crossDomain = settings.crossDomain, data = settings.data, hasParam = false, progress = settings.progress, promise, oldAlways;
        // 处理 xhr 对象，加入 promise 模式支持
        xhr.id = ++id;
        if (crossDomain) {
        }
        ...
        if (typeof data === "object" && _.type(data) !== "formdata" && _.type(data) !== "file") {
            if (type === "get") {
                ...
                _.each(data, function (value, key) {
                    tmp.append(key, _.type(value) === "array" ? value.join() : value);
                });
        ...
        xhr.open(type, settings.url, settings.async, settings.username, settings.password);
    ...
    param = exports.param = function (data, appendTo) {
        ...
        if (typeof appendTo === "string") {
            query = appendTo + (query.length > 0 ? (appendTo.indexOf("?") < 0 ? "?" : "&") + query : "");
        }
        return query;
    };
    // 设置默认选项
    ...
    exports.setDefaultSuccess = function (onSuccess) {
        defaults.onSuccess = typeof onSuccess === "function" ? onSuccess : null;
        return this;
    };
    // 设置默认错误处理函数
    exports.setDefaultError = function (onError) {
        defaults.onError = typeof onError === "function" ? onError : null;
        return this;
    };
    // readystatechange 事件处理函数
    function onReadyStateChange(e) {
        var xhr = e.target, settings = xhr.ajaxSettings || {}, dataType = settings.dataType, status, resText, resBody;
            ...
            if (xhr.status >= 200 && xhr.status < 300 || xhr.status === 304) {
                try {
                    switch (dataType.toLowerCase()) {
                        ...
                        case "xml":
                            //兼容
                            resBody = xhr.responseXML || xhr.responseText;
                            break;
    ...
    function createOptions(args) {
        var p, options = {
            url: args[0]
        }, params = slice.call(args, 1), foundCallBack = false, foundData = false;
        while (params.length) {
            p = params.shift();
            switch (typeof p) {
                case "string":
    ...
    function success(xhr, data) {
        ...
        exports.onSuccessBefore ? exports.onSuccessBefore(args[0]) : '';
        xhr.resolve.apply(xhr, args);
    }

    function error(xhr, msg) {
        var handler = getError(xhr), args;
        msg = typeof msg === "string" ? msg : xhr.statusText;
        args = [xhr, msg, xhr.status];
        handler.apply(xhr, args);
        exports.onErrorBefore ? exports.onErrorBefore(args[0]) : '';
