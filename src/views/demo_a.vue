	...
	<div class="Play-page">
			...
			<div class="picture" v-if='media_type==2'>
				...
				<div class="barrage" id="ul_li">
					<ul>
						...
						<li class="iconfont" v-on:click="pinlun_click" v-if='false'>&#xe61f;<span class="pinglun" >{{pinlun_shu}}</span></li>
						<li id="fenxiang" v-on:click="fenxiang"><img src="../icon/分享 copy.png"></li>
		...
		<main>
			...
			<div v-if='media_type==1' id="iframe">
				<iframe height=200 width=100% :src="content_url" frameborder=0 allowfullscreen></iframe>
				<div class="picture">
			...
			<div class="genduo" v-on:click="genduo" v-if=false>
				<p>查看更多评论</p>
			</div>-->
			<!--<div class="btm">
				<div class="left">
...
<script>
	...
	export default {
		...
		mounted: function() { //挂载时候执行的函数
			...
			//显示上次播放时长
			//alert(this.id);
//			if(localStorage.kecheng_id == this.id){
//				
...
//				var	shifen=fen%10;
//				//拼接时分秒
//				var	sfm=shifen+fen+':'+shimiao+miao;
//				console.log(sfm);
...
//			//监听用户是否离开页面
//			setTimeout(function(){
//				window.onhashchange = function(){myFunction()};
//				function myFunction() {
		...
		methods: {
			...
			$.ajax({
				...
				success: function(data) {
					...
					// console.log(self.time_length);
					// console.log(self.teacher_id);
					// console.log(data.content);
					// console.log(self.content);
					// console.log(self.ppts);
					// console.log(self.parse_num);
					//存储老师id
...
//			this.video=$('#video1');
			...
			//获取评论数
			$.ajax({
					url:"http://weixin.mlxing.com/course_info/comment_row",
					data:{
			...
			$.ajax({
				url:'http://weixin.mlxing.com/course_info/lists',
				data:{
					is_recommend:true,
					page:1	
				},
...
//					if(localStorage.kecheng_id==this.id){
//						if(localStorage.jindu){
//							var adoMain=$('#adoMain');
//							adoMain[0].currentTime=localStorage.jindu;
//						}
//					}
...
//					},1000);
					...
					// console.log("暂停了");
					//清除播放进度，计时器
					//clearInterval(this.adf);
					console.log(this.adoMain[0].currentTime);
				}
			},
			//上一首
			retreat: function() {
...
//				}
			...
			dianzan:function(){
					...
					$.ajax({
					url: 'http://weixin.mlxing.com/sys_biz_set/praise_insert',
					data: {
			...
			shouchang:function(){
				...
				$.ajax({
					url: 'http://weixin.mlxing.com/sys_biz_set/collect_insert',
					data: {
//						"biz_id":"self.id",
...
//				location.hash='#/reward';
				var self = this;
				this.$router.push({name:'reward',params:{biz_id:self.id,be_reward_user:self.teacher_id}});
			},
			//记录当跳出页面时的播放进度
//			bofang_jindu:function(){
//				this.adoMain = $('#adoMain');
//				var bofangjd=this.adoMain[0].currentTime;
//				alert();
//			},
