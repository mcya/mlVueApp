...
var sTermInfo = new Array(0,21208,42467,63836,85337,107014,128867,150921,173149,195551,218072,240693,263343,285989,308563,331033,353350,375494,397447,419210,440795,/*462224,*/462924,484232,504758);
...
function drawCld(SY,SM) {
      ...
      if(sD>-1 && sD<cld.length) { //日期内
		...
		for (var ipp=0;ipp<lFtv.length;ipp++){	//农历节日
			...
			if (12==(cld[sD].lMonth)){	//判断是否为除夕
				if (eve==(cld[sD].lDay)){lObj[i]="除夕";Slfw="除夕";}
	    ...
	    if ((SM+1)==5){	//母亲节
			if (fat==0){
				if ((sD+1)==7){Ssfw="母亲节";lObj[i]="母亲节"}
			}
			else if (fat<9){
				if ((sD+1)==((7-fat)+8)){Ssfw="母亲节";lObj[i]="母亲节"}
			}
		}
	    if ((SM+1)==6){	//父亲节
			if (mat==0){
				if ((sD+1)==14){Ssfw="父亲节";lObj[i]="父亲节"}
			}
			else if (mat<9){
				if ((sD+1)==((7-mat)+15)){Ssfw="父亲节";lObj[i]="父亲节"}
			}
		 }
      }
      else { //非日期
   ...
   return {date:sObj,nong:lObj};
}
//在下拉列表中选择年月时,调用自定义函数drawCld(),显示公历和农历的相关信息
function changeCld() {
   console.log('todo222')
   tY=vm.changeTime.getFullYear();
   tM=vm.changeTime.getMonth();
   tD=vm.changeTime.getDate();
   console.log(tY,tM)
   return drawCld(tY,tM);
}
//用自定义变量保存当前系统中的年月日
var Today = new Date();
var tY;
